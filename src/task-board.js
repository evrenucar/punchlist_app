    ...
    function saveState() {
      ...
      if (syncable) state.rev = (Number(state.rev) || 0) + 1;
    ...
    function syncDecision({ remoteExists, remoteSha, lastSha, dirty, base, localRev, remoteRev }) {
      ...
      if (knownRemote && localRev > remoteRev) return "push";          // we have more edits -> we win, remote kept in git
      if (knownRemote && localRev < remoteRev) return "pull";          // remote has more edits -> adopt it
    ...
    async function syncNow(trigger) {
      ...
      try {
        ...
        if (action === "conflict") {
          ...
          resolved = window.confirm("This device and another both changed since the last sync, and neither is clearly newer. Keep THIS device's version? (Cancel keeps the other device's version. Either way the other stays in Undo and the repo history.)")
            ? "push"
            : "pull";
        }
        if (resolved === "pull") {
    ...
    // A rendered offset that lands on a style span's closing edge is ambiguous:
    // inside the span or just past it. Chrome keeps the caret inside, so the
    // next character joins the bold (Evren: "bold ** are sticky at the end of
    // the line"). Step outside instead — leaving a style should end it.
    // Moving the range out is not enough: Chrome takes the typing style from
    // the character BEFORE the caret wherever the range sits, so the caret
    // needs a character of its own to stand after. That is CARET_BOUNDARY, a
    // zero-width node the serializers already strip and the next render drops.
    function escapeClosingStyleEdge(node, element) {
      let edge = node;
      while (edge.parentNode && edge.parentNode !== element
        && edge.parentNode.lastChild === edge
        && INLINE_STYLE_TAGS[edge.parentNode.tagName?.toLowerCase()]) edge = edge.parentNode;
      return edge === node ? null : edge;
    }

    // ...and the boundary must never spend a keystroke of its own. Backspace
    // eating an invisible character reads as a dead key, so any non-typing key
    // drops it first and inherits its spot.
    function dropCaretBoundary() {
      const selection = window.getSelection?.();
      if (!selection || !selection.rangeCount || !selection.isCollapsed) return;
      const range = selection.getRangeAt(0);
      const node = range.startContainer;
      if (node.nodeType !== 3 || node.nodeValue !== CARET_BOUNDARY || range.startOffset !== 1) return;
      const parent = node.parentNode;
      const at = [...parent.childNodes].indexOf(node);
      parent.removeChild(node);
      const next = document.createRange();
      next.setStart(parent, at);
      next.collapse(true);
      selection.removeAllRanges();
    ...
    function placeCaretAtTextOffset(element, offset) {
      ...
      while ((node = walker.nextNode())) {
        if (remaining <= node.textContent.length) {
          ...
          const edge = remaining === node.textContent.length ? escapeClosingStyleEdge(node, element) : null;
          if (edge) {
            const boundary = document.createTextNode(CARET_BOUNDARY);
            edge.parentNode.insertBefore(boundary, edge.nextSibling);
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key.length !== 1) dropCaretBoundary();
