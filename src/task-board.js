    ...
    function loadStateFromLocalStorage() {
      ...
      try {
        ...
        if (!Array.isArray(parsed.groups)) return normalizeState(seedState());
    ...
    function findTask(id, tasks = null, parent = null) {
      if (!tasks) {
        for (const group of state.groups) {
          const found = findTask(id, group.tasks, parent);
      ...
      for (let index = 0; index < list.length; index += 1) {
        ...
        if (item.id === id) return { item, parent, list, index };
        const child = findTask(id, item.children || [], item);
    ...
    function normalizeState(boardState) {
      boardState.groups.forEach((group, index) => {
        ...
        group.tasks.forEach(normalizeTask);
      });
      return boardState;
    }
    ...
    function normalizeTask(item) {
      ...
      item.children.forEach(normalizeTask);
      return item;
    }
