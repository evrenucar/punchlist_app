    ...
    function setTaskCompleted(id, done, now = new Date().toISOString(), options = {}) {
      ...
      if (options.render !== false) {
        if (nextDone && isTaskHiddenFromActive(found.item, found.group)) {
          ...
          renderGroupInPlace(found.group.id);
        } else {
          render();
        }
      }
    ...
    function deleteTaskWithPolicy(id, now = new Date().toISOString(), options = {}) {
      ...
      if (options.render !== false) {
        if (scopedOk) {
          renderGroupInPlace(scopedGroupId);
    ...
    function applySwipeIndent(id, levels) {
      ...
      const groupId = findTask(id)?.group?.id;
      let applied = 0;
    ...
    // Sweep fill: fast finger moves skip rows between two elementFromPoint
    // samples, so take every visible node from the last swept row through here.
    function extendSelectionThrough(node, fromNode = null) {
      const visible = getVisibleNodes();
      const from = visible.findIndex((item) => sameNode(item, fromNode));
      const to = visible.findIndex((item) => sameNode(item, node));
      if (from < 0 || to < 0) {
        addNodeToSelection(node);
        return;
      }
      const step = to >= from ? 1 : -1;
      for (let i = from; i !== to + step; i += step) addNodeToSelection(visible[i]);
    }
    ...
    function insertSiblingBelowNode(node = selectedNode) {
      ...
      else renderGroupInPlace(found.group.id);
      return item;
    }
    ...
    function indentTask(id, options = {}) {
      ...
      if (options.save !== false) saveState();
      if (options.render !== false) renderGroupInPlace(found.group.id);
      return true;
    }
    ...
    function addTask(groupId, parentId = null) {
      ...
      setSingleSelection({ kind: "task", id: item.id });
      saveState();
      if (scopedOk) renderGroupInPlace(groupId);
      else render();
    ...
    function mergeTaskIntoPrevious(id) {
      ...
      if (taskIsLinkFree(prevFound.item) && !getLinkCount(item.id)) renderGroupInPlace(found.group.id);
      else render();
      return true;
    }
    ...
    function getTaskSplitPlan(text, offset) {
      ...
      // Enter at the very start creates an empty item BELOW and leaves the
      // original intact (Evren 2026-07-19) — it used to push an empty row
      // above and move his caret up into it
      if (caret === 0) return { beforeText: source, afterText: "", position: "start" };
      return {
    ...
    function splitTaskAtOffset(id, offset) {
      ...
      const intoChildren = plan.afterText === ""
        && !resolvedSplit.collapsed && (resolvedSplit.children || []).length > 0;
      const newItem = task(plan.afterText, [], {
      ...
      if (taskIsLinkFree(found.item)) renderGroupInPlace(found.group.id);
      else render();
    ...
    boardEl.addEventListener("pointermove", (event) => {
      ...
      if (!touchSelect.armed) {
        if (shouldCancelLongPress(touchSelect.startX, touchSelect.startY, event.clientX, event.clientY)) clearTouchSelect();
        return;
      }
      event.preventDefault();
      updateDragAutoScroll(event.clientY);
      ...
      extendSelectionThrough(node, touchSelect.lastNode);
