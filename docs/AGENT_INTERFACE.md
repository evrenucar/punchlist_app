...
- Task fields the wrapper understands: `by` (`agent`/`user`/`seed` — authorship colors; never strip), `lane` (`dev`/`app` — the yellow/blue node tags; set on every item you create), `active` (purple glow + ACTIVE bar + graph anchor; keep truthful), `inserted` (priority-insertion marker; since 2026-07-19 it no longer routes to the graph's side branch — the graph branches ONLY items whose `createdUnderTaskId` is a currently-active item, so set that field when work genuinely forks off the active thread).
