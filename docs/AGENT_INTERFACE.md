...
- `node status/agent-heartbeat.mjs <name> <taskId> [status...]` — presence: a named chip on the graph node plus task/runtime/status in the page header. Re-run with new status text at every phase change; the runtime clock survives. Killing: TaskStop only kills the shell wrapper on Windows — kill the node child by PID (`wmic process where "commandline like '%agent-heartbeat%'" get processid`, which matches its own pipeline, so trust `GET /agents` = `[]` as ground truth), then `--stop`.
