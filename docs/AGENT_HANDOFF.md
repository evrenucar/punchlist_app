...
**THE FLASH (task-mrred11c) is root-caused with measurements** (full diagnosis in the 2026-07-21 chat/board): 150-700ms main-thread freezes, quadratic getLinkCount (fixed on main — one walk per render, see linkCountCache), alias fanout full-renders (his Ctrl+D day set is aliases), top-level edits rebuilding whole huge groups, delete-confirm and slide-away full renders. **MID-FLIGHT AT STOP: a surgery agent was implementing ranked patches 2-5 in worktree branch `worktree-agent-a2ddb609c3de159d6`** — next agent: check `git worktree list` / that branch for a committed result, REVIEW it (scoped-render traps in the notes below), merge via rebuild-from-merged-sources (generated files always conflict; take either side then rebuild), browser-verify on a seeded 1.4k board, and get his on-glass verdict. iOS image-decode flash and keyboard flicker remain unverified on glass.
