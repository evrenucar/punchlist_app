...
Considered and set aside: `web-performance-audit` and `web-performance-optimization`. The app is one 460 KB file with zero network requests; there is no bundle to split and no waterfall to fix. Revisit only if board render time on the phone becomes a reported friction.
